...
import type { TrainersPageContentResolved } from '@/marketing/constants/trainersPageConstants';

const JOIN_EMAIL = 'coaches@camsservices.co.uk';
  ...
  return (
    <div>
      <Section className="relative pt-16 pb-16 px-4 sm:px-6 lg:px-8 text-white overflow-hidden min-h-[40vh] flex items-center">
        ...
        <div className="relative text-center max-w-4xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-5">
            ...
            <MarketingButton href={content.hero.ctaSecondaryHref} variant="outline" size="lg" className="shadow-lg border-white/30 text-white hover:bg-white/10" withArrow>
        ...
        <div className="container mx-auto px-4">
          <div className="flex flex-wrap justify-center items-center gap-6 md:gap-8 text-center">
            <div>
              ...
              <p className="text-sm opacity-90">{content.stats.mentorsLabel}</p>
            </div>
            <div className="hidden sm:block w-px h-12 bg-white/30" />
            <div>
              <p className="text-4xl font-bold">{averageRating > 0 ? averageRating.toFixed(1) : '4.9'}/5</p>
              <p className="text-sm opacity-90">{content.stats.ratingLabel}</p>
            </div>
            <div className="hidden sm:block w-px h-12 bg-white/30" />
            <div>
              <p className="text-4xl font-bold">100%</p>
              <p className="text-sm opacity-90">{content.stats.dbsLabel}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="py-20 bg-gradient-to-br from-blue-50 to-white">
        <Section title={content.section.title} subtitle={content.section.subtitle} titleClassName="heading-text-shadow">
          {loading ? (
          ...
          <div className="max-w-5xl mx-auto">
            <div className="bg-white rounded-card border-2 border-primary-blue/20 p-8 md:p-12 shadow-card">
              <div className="text-center mb-8">
                ...
                <h2 className="text-4xl md:text-5xl font-heading font-bold text-navy-blue mb-4 heading-text-shadow">{content.joinSection.title}</h2>
                <p className="text-xl text-navy-blue/80 max-w-3xl mx-auto leading-relaxed">{content.joinSection.subtitle}</p>
              </div>
              ...
              <div className="flex flex-col sm:flex-row justify-center gap-4">
                <MarketingButton href={content.joinSection.ctaPrimaryHref} variant="superPlayful" size="lg" className="shadow-lg flex items-center justify-center gap-2" withArrow>
                  {content.joinSection.ctaPrimary}
                </MarketingButton>
                <MarketingButton href={content.joinSection.ctaSecondaryHref} variant="primary" size="lg" className="shadow-lg flex items-center justify-center gap-2" withArrow>
                  {content.joinSection.ctaSecondary}
                </MarketingButton>
              </div>

              <div className="mt-8 text-center">
                <p className="text-sm text-navy-blue/80">
                  {content.joinSection.contactEmailText}{' '}
                  <a href={`mailto:${JOIN_EMAIL}`} className="text-primary-blue font-semibold hover:underline">
                    {JOIN_EMAIL}
                  </a>
                </p>
              </div>
      ...
      <CTASection
        ...
        primaryCTA={{ text: content.cta.primaryText, href: content.cta.primaryHref }}
        secondaryCTA={{ text: content.cta.secondaryText, href: content.cta.secondaryHref }}
        variant="default"
      />
    </div>
  );
